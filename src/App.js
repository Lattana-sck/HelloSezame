...
function App() {
  ...
  const calc = () => {
    ...
    setAdditionalSavings(Math.round((0.1 * (homecost - deposit)) / 36));
    setMonthlyBudget(rent + additionalSavings);
  };

  const fetchData = async () => {
    fetch(`https://crazy_wiles:3000/get`)
    ...
    <div className="App flex flex-col min-h-screen">
      <header>
        <div className="shadow-2xl bg-blue-900 text-white h-1/4 p-6">
          ...
          <span className="font-bold text-2xl">We'll do the math for you.</span>
          <br />
      ...
      <div className="shadow-2xl border border-black-800 rounded flex flex-col w-1/4 p-6 m-auto mt-6 text-center justify-center content-center">
        <h1 className="font-bold text-lg">Home Details</h1>
        <br />
        <label>Type your city : </label>
        <input
          id="city"
          name="city"
          className="px-4 bg-slate-100 rounded-xl border border-slate-200 hover:border-slate-400"
        ...
        />
        <button
          className="m-6 bg-blue-900 hover:bg-blue-600 text-white rounded-xl border border-black"
          onClick={() => calc()}
        >
          Calculate
        </button>
